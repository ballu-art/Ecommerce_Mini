...
export class ProductService {
  ...
  private generateSlug(name: string): string {
    return name
      ...
      .slice(0, 100);            // Limit slug length for URL
  }

  /**
   * Add slug to all products if not already present
   */
  private enrichWithSlugs(products: Product[]): Product[] {
    return products.map(p => ({
      ...p,
      slug: p.slug || this.generateSlug(p.name)
    }));
  }
  loadProducts(): void {
    ...
    if (savedProducts && savedProducts.length > 0) {
      const enriched = this.enrichWithSlugs(savedProducts);
      this.productsSubject.next(enriched);
      console.log(`Loaded ${enriched.length} products from localStorage`);
      return;
    }
    ...
    this.http.get<Product[]>(this.PRODUCTS_JSON_URL).subscribe({
      next: (products) => {
        const enriched = this.enrichWithSlugs(products);
        this.productsSubject.next(enriched);
        this.saveToLocalStorage(enriched);
        console.log(`Loaded ${enriched.length} products from products.json`);
      },
      error: (error) => {
  ...
  getBySlug(slug: string): Observable<Product | undefined> {
    return new Observable(observer => {
      const product = this.productsSubject.value.find(p => p.slug === slug);
      observer.next(product);
      observer.complete();
    });
  }
  ...
  /**
   * Get products by category (validated against APP_CONSTANTS.CATEGORIES)
   */
  getByCategory(category: string): Observable<Product[]> {
    return new Observable(observer => {
      ...
      if (!actualCategory) {
        console.warn(`Category "${category}" not found in APP_CONSTANTS.CATEGORIES`);
        observer.next([]);
        observer.complete();
        return;
      }
